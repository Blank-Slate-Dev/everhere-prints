...
export default function SoundWavePrintPreview({
  ...
  // Use FULL waveform for the print (resampled to consistent size)
  const displayWaveform = useMemo(() => {
    if (!hasAudio || waveformData.length === 0) {
      return generatePlaceholderWaveform();
    }
    
    // Resample full waveform to consistent number of bars for display
    ...
    for (let i = 0; i < targetBars; i++) {
      ...
      resampled.push(waveformData[idx]);
    }
    return resampled;
  ...
  // Playhead position for SVG (0-400 range)
    ...
    >
      <div className="relative mx-auto max-w-md">
        ...
        <div
          ...
          style={{ transform: "translate(6px, 10px)", filter: "blur(25px)", background: config.shadowColor, opacity: 0.7 }}
        />
        ...
        >
          ...
          <div
            ...
            style={{ aspectRatio: `${paperAspectRatio}`, backgroundColor: style.backgroundColor }}
          >
            <div className="absolute inset-0 flex flex-col" style={{ padding: "6%" }}>
              
              {/* Layout with Album Art */}
              {displayAlbumArt && songData?.albumArtUrl ? (
                <>
                      ...
                      {style.waveGradient && (
                        <defs>
                          <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
                            <stop offset="0%" stopColor={style.waveGradient.from} />
                            <stop offset="50%" stopColor={style.waveGradient.to} />
                            <stop offset="100%" stopColor={style.waveGradient.from} />
                          </linearGradient>
                        </defs>
                      )}
                  ...
                  <div className="flex-grow flex flex-col items-center justify-center text-center" style={{ paddingTop: "2%" }}>
                    <motion.h1
                      ...
                      style={{ color: style.textColor, fontSize: "clamp(12px, 3.5vw, 20px)", marginBottom: "2%" }}
                    >
                      {title || songData?.songName || "Your Song"}
                    </motion.h1>

                    {displayArtistName && songData && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-light tracking-widest uppercase" style={{ color: style.textColor, opacity: 0.7, fontSize: "clamp(7px, 2vw, 10px)", letterSpacing: "0.2em", marginBottom: "2%" }}>
                        {songData.artistName}
                      </motion.p>
                    )}

                    {displayAlbumName && songData && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="italic" style={{ color: style.accentColor, fontSize: "clamp(6px, 1.8vw, 9px)", marginBottom: "2%" }}>
                        {songData.albumName}
                      </motion.p>
                    )}

                    {displayDuration && durationDisplay && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-mono tracking-widest" style={{ color: style.accentColor, fontSize: "clamp(9px, 2.5vw, 12px)", opacity: 0.6 }}>
                        {durationDisplay}
                      </motion.p>
                    )}

                    {dateText && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="tracking-widest uppercase" style={{ color: style.accentColor, fontSize: "clamp(5px, 1.5vw, 8px)", letterSpacing: "0.25em", marginTop: "3%" }}>
                        {dateText}
                      </motion.p>
                    )}
                  </div>
                ...
                <>
                  ...
                  <div className="flex-shrink-0 text-center" style={{ marginBottom: "3%" }}>
                    <motion.h1 key={title} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-serif font-semibold tracking-wide" style={{ color: style.textColor, fontSize: "clamp(14px, 4.5vw, 24px)" }}>
                      {title || "Your Sound Wave"}
                    </motion.h1>
                    
                    {subtitle && !displayArtistName && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-light tracking-widest uppercase mt-2" style={{ color: style.textColor, opacity: 0.7, fontSize: "clamp(7px, 2vw, 10px)", letterSpacing: "0.15em" }}>
                        {subtitle}
                      </motion.p>
                    )}
                  </div>
                  ...
                  <div className="flex-shrink-0 text-center" style={{ marginTop: "3%" }}>
                    {displayDuration && durationDisplay && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-mono tracking-widest" style={{ color: style.accentColor, fontSize: "clamp(10px, 3vw, 16px)", opacity: 0.6, marginBottom: "2%" }}>
                        {durationDisplay}
                      </motion.p>
                    )}

                    {dateText && (
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="tracking-widest uppercase" style={{ color: style.accentColor, fontSize: "clamp(6px, 1.8vw, 9px)", letterSpacing: "0.25em" }}>
                        {dateText}
                      </motion.p>
                    )}
                  </div>
