      ...
      <div className="relative mx-auto max-w-md">
        ...
        <motion.div
          ...
          style={{
            ...
            boxShadow: hasFrame ? `4px 8px 25px -5px ${config.shadowColor}, 8px 16px 40px -10px rgba(0,0,0,0.25)` : `4px 8px 25px -8px rgba(0,0,0,0.3)`,
          }}
        >
          {hasFrame && (
            <>
          ...
          <div className="relative bg-white" style={{ padding: hasFrame ? "8px" : "12px", boxShadow: hasFrame ? `inset 1px 1px 3px rgba(0,0,0,0.04)` : "none" }}>
            ...
            <div className="relative aspect-square w-full bg-white">
              <Image src={colorConfig.image} alt={`New Zealand Map - ${colorConfig.name}`} fill className="object-contain" priority />

              {showPin && pinPosition && pinPosition.isValid && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute z-10" style={{ left: `${pinPosition.x}%`, top: `${pinPosition.y}%` }}>
                  <motion.div initial={{ y: -50, scale: 0.5 }} animate={{ y: 0, scale: 1 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} style={{ position: "absolute", left: "-14px", top: "-28px" }}>
              ...
              {location && (
                <div className="absolute top-2 left-2 text-[8px] md:text-[9px] font-mono pointer-events-none z-20" style={{ color: colorConfig.accentColor, opacity: 0.7 }}>
      ...
      <div ref={captureRef} aria-hidden="true" style={{ position: "absolute", left: "-9999px", top: 0, width: "300px", backgroundColor: "#ffffff" }}>
        <div style={{ padding: hasFrame ? "10px" : "0px", backgroundColor: captureFrameColor, borderRadius: "2px" }}>
          <div style={{ padding: hasFrame ? "6px" : "8px", backgroundColor: "#ffffff" }}>
            ...
            <div style={{ position: "relative", width: "100%", aspectRatio: "1", backgroundColor: "#ffffff", overflow: "hidden" }}>
              <img src={colorConfig.image} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "contain" }} crossOrigin="anonymous" />
              {pinPosition && pinPosition.isValid && (
                <div style={{ position: "absolute", left: `${pinPosition.x}%`, top: `${pinPosition.y}%`, zIndex: 10 }}>
                  <div style={{ position: "absolute", left: "-8px", top: "-16px", width: "16px", height: "16px" }}>
              ...
              {location && <div style={{ position: "absolute", top: "4px", left: "4px", fontSize: "5px", fontFamily: "monospace", color: colorConfig.accentColor, opacity: 0.7 }}>{Math.abs(location.latitude).toFixed(4)}°S, {location.longitude.toFixed(4)}°E</div>}
            </div>
            {/* Text */}
            <div style={{ padding: "8px 4px", textAlign: "center" }}>
              <p style={{ fontSize: "6px", textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: "2px", color: colorConfig.accentColor, margin: "0 0 2px 0" }}>{title || "Our Special Place"}</p>
              <p style={{ fontFamily: "Georgia, serif", fontSize: "10px", lineHeight: 1.2, color: colorConfig.textColor, margin: 0 }}>{location ? formatLocationName(location.placeName) : "Your Location"}</p>
              {(subtitle || date) && <p style={{ fontSize: "6px", marginTop: "2px", color: colorConfig.accentColor, margin: "2px 0 0 0" }}>{subtitle}{subtitle && date && " • "}{date}</p>}
            </div>
          </div>
        </div>
      </div>
