...
import { MapLocation, PrintSize, FrameOption, StarMapCustomization, StarMapProductSelection } from "@/types";
...
export default function CreateStarMapPage() {
  ...
  const handleTimeChange = useCallback((time: string) => { setCustomization((prev) => ({ ...prev, time })); }, []);
  ...
  const handleToggleConstellations = useCallback((value: boolean) => { setCustomization((prev) => ({ ...prev, showConstellations: value })); }, []);
  const handleToggleConstellationNames = useCallback((value: boolean) => { setCustomization((prev) => ({ ...prev, showConstellationNames: value })); }, []);
  const handleToggleGrid = useCallback((value: boolean) => { setCustomization((prev) => ({ ...prev, showGrid: value })); }, []);
  const handleToggleMilkyWay = useCallback((value: boolean) => { setCustomization((prev) => ({ ...prev, showMilkyWay: value })); }, []);
  const handleToggleCardinals = useCallback((value: boolean) => { setCustomization((prev) => ({ ...prev, showCardinals: value })); }, []);
  ...
  return (
    <div className="min-h-screen pt-20 lg:pt-24 bg-cream">
      <AnimatePresence>
        {showMiniPreview && <StarMapMiniPreview customization={customization} product={product} onTap={scrollToPreview} />}
      </AnimatePresence>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8 lg:mb-12">
          <h1 className="text-3xl lg:text-4xl font-serif font-semibold text-charcoal">Create Your Star Map</h1>
          <p className="mt-3 text-brand-600 max-w-xl mx-auto">Capture the night sky exactly as it appeared on your special moment. Perfect for anniversaries, births, and unforgettable nights.</p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-start">
          <motion.div ref={previewContainerRef} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="lg:sticky lg:top-28">
            <StarMapPrintPreview customization={customization} product={product} captureRef={captureRef} />
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="space-y-8">
            ...
            <div className="bg-white rounded-2xl p-6 border border-brand-100 shadow-sm">
              <h2 className="text-lg font-semibold text-charcoal mb-4">2. Choose Your Date & Time</h2>
              <StarMapDateTimePicker date={customization.date} time={customization.time} onDateChange={handleDateChange} onTimeChange={handleTimeChange} />
            </div>

            <div className="bg-white rounded-2xl p-6 border border-brand-100 shadow-sm">
              <h2 className="text-lg font-semibold text-charcoal mb-4">3. Choose Your Style</h2>
              <div className="space-y-6">
                <StarMapStyleSelector selectedStyleId={customization.styleId} onStyleChange={handleStyleChange} />
                <StarMapDisplayOptions showConstellations={customization.showConstellations} showConstellationNames={customization.showConstellationNames} showGrid={customization.showGrid} showMilkyWay={customization.showMilkyWay} showCardinals={customization.showCardinals} onToggleConstellations={handleToggleConstellations} onToggleConstellationNames={handleToggleConstellationNames} onToggleGrid={handleToggleGrid} onToggleMilkyWay={handleToggleMilkyWay} onToggleCardinals={handleToggleCardinals} />
              </div>
            </div>

            <div className="bg-white rounded-2xl p-6 border border-brand-100 shadow-sm">
              <h2 className="text-lg font-semibold text-charcoal mb-4">4. Personalise Your Text</h2>
              <StarMapTextEditor title={customization.title} subtitle={customization.subtitle} dateText={customization.dateText} onTitleChange={handleTitleChange} onSubtitleChange={handleSubtitleChange} onDateTextChange={handleDateTextChange} />
            </div>

            <div className="bg-white rounded-2xl p-6 border border-brand-100 shadow-sm">
              <h2 className="text-lg font-semibold text-charcoal mb-4">5. Select Size & Frame</h2>
              <StarMapProductOptions selectedSize={product.size} selectedFrame={product.frame} onSizeChange={handleSizeChange} onFrameChange={handleFrameChange} />
            </div>

            <StarMapOrderSummary customization={customization} product={product} previewRef={captureRef} />
          </motion.div>
        </div>
      </div>
    </div>
