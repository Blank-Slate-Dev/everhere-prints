// src/app/calibrate-newzealand/page.tsx
"use client";
...
import {
  newzealandMapColors,
  getNewZealandMapColor,
} from "@/lib/newzealandMapConfig";
...
export default function CalibrateNewZealandPage() {
  ...
  const colorConfig = getNewZealandMapColor(selectedColorId);
  ...
  const calculateBounds = useCallback((positions: PinPositions) => {
    const capeReinga = positions.capeReinga || defaultPinPositions.capeReinga;
    const eastCape = positions.eastCape || defaultPinPositions.eastCape;
    const bluff = positions.bluff || defaultPinPositions.bluff;
    const westCape = positions.westCape || defaultPinPositions.westCape;

    // Calculate relative positions in geographic space
    const capeReingaRelY = (calibrationPoints[0].lat - GEO_BOUNDS.north) / (GEO_BOUNDS.south - GEO_BOUNDS.north);
    const bluffRelY = (calibrationPoints[2].lat - GEO_BOUNDS.north) / (GEO_BOUNDS.south - GEO_BOUNDS.north);
    const westCapeRelX = (calibrationPoints[3].lng - GEO_BOUNDS.west) / (GEO_BOUNDS.east - GEO_BOUNDS.west);
    const eastCapeRelX = (calibrationPoints[1].lng - GEO_BOUNDS.west) / (GEO_BOUNDS.east - GEO_BOUNDS.west);

    const height = (bluff.y - capeReinga.y) / (bluffRelY - capeReingaRelY);
    const top = capeReinga.y - capeReingaRelY * height;
    const bottom = top + height;

    const width = (eastCape.x - westCape.x) / (eastCapeRelX - westCapeRelX);
    const left = westCape.x - westCapeRelX * width;
  ...
  const generateCurrentConfig = () => {
    ...
    const pinLines = calibrationPoints
      .map((point) => {
...
${pinLines}
  ...
  const generateAllConfig = () => {
    ...
    newzealandMapColors.forEach((color) => {
...
${pinLines}
    ...
    return `// New Zealand Map Calibration Data
    ...
    <div className="min-h-screen pt-24 pb-12 bg-gray-100">
      <div className="max-w-6xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-center mb-2">
          New Zealand Map Calibration
        </h1>
        <p className="text-center text-gray-600 mb-6">
          Calibrate each map color separately. Drag pins to match locations on
          the watercolor map.
        </p>
        ...
        <div className="bg-white rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm text-gray-500">
              {calibratedColors.size} / {newzealandMapColors.length} calibrated
            </span>
          </div>
          ...
          <div className="bg-white rounded-xl p-4 flex items-center gap-3">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-sm ${
                showLabels ? "bg-gray-900 text-white" : "bg-gray-200"
              }`}
            >
              {showLabels ? "On" : "Off"}
            </button>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 max-w-md mx-auto">
          ...
          >
            ...
            {calibrationPoints.map((point) => {
              const pos =
                currentPinPositions[point.id] || defaultPinPositions[point.id];
        ...
        <div className="mt-8 bg-white rounded-xl p-6 max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold">
              Full Config for {colorConfig.name}:
            </h2>
            <button
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="font-semibold text-green-800">
                  Export All Calibrated Maps
                </h2>
                <p className="text-sm text-green-600">
                  {calibratedColors.size} maps with full pin data
                </p>
              </div>
              <button
